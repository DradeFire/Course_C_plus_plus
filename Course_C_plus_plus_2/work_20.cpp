...
void PrintStats(vector<Person> persons) {
    ...
    auto it1 = partition(persons.begin(), persons.end(), [](Person p) {
            return p.gender == Gender::FEMALE;
        });
    cout << "Median age for females = " << ComputeMedianAge(begin(persons), it1) << endl;
    
    auto it2 = partition(persons.begin(), persons.end(), [](Person p) {
        return p.gender == Gender::MALE;
        });
    cout << "Median age for males = " << ComputeMedianAge(begin(persons), it2) << endl;
    
    auto it3 = partition(persons.begin(), persons.end(), [](Person p) {
    ...
    auto it4 = partition(persons.begin(), persons.end(), [](Person p) {
